            ...
            <Table responsive hover className="my-3">
              <thead className="his-thead">
                <tr className="text-center">
                  ...
                  <th>Isi</th>
                  <th>Status</th>
                  {user && user.role !== 'user' && <th>Aksi</th>}
                </tr>
              </thead>
              <tbody>
                {/* {Object.keys(messages).map((message) => message.data.map((data1, index) => ( */}
                {messages.map((message, index) => (
                  <tr
                    className="hover-his text-center"
                    key={message.message_id}
                  >
                    <td>{index + 1}</td>
                    <td>{message.senderName}</td>
                    <td>{message.message_type}</td>
                    <td className="text-break">{message.message_content}</td>
                    <td>{message.status}</td>
                    {user && user.role !== 'user' && (
                      <td>
                        <Link to={`/message/update/${message.message_id}`}>
